          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <div className="flex gap-2">
                <button className="bg-blue-600 px-3 py-2 rounded">Send Inquiry</button>
                <button className="bg-slate-800 px-3 py-2 rounded">Save</button>
              </div>
            </div>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-md overflow-hidden">
          <div className="bg-slate-900 border-b border-slate-700 p-4 flex justify-between items-center">
            ...
            <button onClick={() => { setShowTaskDetail(false); setSelectedTask(null); }} className="p-2">
              <X size={20} />
            </button>
          </div>
          <div className="p-6">
            <div className="mb-4 flex items-center gap-4">
              <span className={`px-2 py-1 text-xs rounded ${selectedTask.status === "completed" ? "bg-green-900 text-green-400" : selectedTask.status === "in-progress" ? "bg-yellow-900 text-yellow-400" : "bg-red-900 text-red-400"}`}>
                {selectedTask.status}
              </span>
        ...
        <div className="bg-slate-900 border border-slate-700 w-full max-w-6xl rounded-md overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-700 flex gap-2">
            {["overview", "team", "vendors", "tasks", "budget", "guests"].map(tabName => (
              <button key={tabName} onClick={() => setTab(tabName)} className={`px-3 py-2 text-sm capitalize ${tab === tabName ? "bg-blue-600 text-white" : "text-slate-400 hover:bg-slate-800"}`}>
                {tabName}
              </button>
            ))}
          </div>
          <div className="p-6 max-h-[70vh] overflow-y-auto">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="w-full bg-slate-800 h-3 rounded">
                        <div className="bg-blue-600 h-full" style={{ width: `${pct(selectedEvent.completed, selectedEvent.tasks)}%` }} />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between mb-2">
                      ...
                      <div className="w-full bg-slate-800 h-3 rounded">
                        <div className="bg-green-600 h-full" style={{ width: `${pct(selectedEvent.spent, selectedEvent.budget)}%` }} />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {/* Additional tabs like budget, guests can be added similarly */}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        {activeTab === "dashboard" && (
          <div>
            {/* Dashboard code (same as above, expanded with demo data) */}
          </div>
        )}
        {activeTab === "events" && (
          <div>
            {/* Events code (same as above, expanded with demo data) */}
          </div>
        )}
        {activeTab === "vendors" && (
          <div>
            {/* Vendors code (search + filter + list) */}
          </div>
        )}
        {activeTab === "tasks" && (
          <div>
