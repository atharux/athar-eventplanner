...
{activeEventTab === 'schedule' && selectedEvent && localSchedule && (
  (() => {
    const parseHour24 = (t) => {
      if (!t) return 9;
    ...
    const normalized = scheduleData.map((it, idx) => {
      const startH = parseHour24(it.time);
      const start = startH === 0 ? 24 : startH;
      const dur = parseDurationHours(it.duration);
      return {
        ...it,
        _idx: idx,
        _start: start - TIMELINE_START_HOUR,
        _dur: dur,
        _end: start + dur,
        _color: colors[it.category] || '#64748b',
      };
    });

    const ticks = Array.from({ length: TIMELINE_HOURS + 1 }, (_, i) => {
      let h = TIMELINE_START_HOUR + i;
      if (h >= 24) h = 0;
      return `${String(h).padStart(2, '0')}:00`;
    });

    const nowPct =
      ((new Date().getHours() + new Date().getMinutes() / 60 - TIMELINE_START_HOUR) / TIMELINE_HOURS) * 100;

    const handleEdit = (it) => {
      if (typeof setSelectedTask === 'function' && typeof setShowTaskDetail === 'function') {
        setSelectedTask(it);
        setShowTaskDetail(true);
      } else if (typeof setShowAddScheduleModal === 'function') {
    ...
    return (
      <div className="space-y-6 relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className={`${classes.panelBg} ${classes.border} p-3 rounded-md`}>
        ...
        <div className="flex gap-4">
          {/* Left List */}
          <div className={`${classes.panelBg} ${classes.border} rounded-md`} style={{ minWidth: 320 }}>
            ...
            {normalized.map((it) => (
              <div
                key={it._idx}
                className="flex items-center justify-between px-4 py-3 hover:bg-slate-800 group"
              >
                <div>
...
>
  <Edit size={14} className="text-slate-300" />
          ...
          <div className="flex-1 overflow-x-auto relative">
            <div
              className={`${classes.panelBg} ${classes.border} rounded-md p-3 relative`}
              style={{ minWidth: 900, boxShadow: neonBoxShadow }}
            >
              <div className="flex text-xs text-slate-400 mb-2">
                {ticks.map((t, i) => (
                  <div key={i} className="flex-1 text-center border-r border-slate-800 min-w-[56px]">
                    {t}
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3 relative">
                {normalized.map((it) => {
                  const left = (it._start / TIMELINE_HOURS) * 100;
                  const width = (it._dur / TIMELINE_HOURS) * 100;
                  return (
                    <div key={it._idx} className="relative h-10">
                      <div
                        className="absolute top-1 h-8 rounded-md flex items-center px-3 text-xs text-white cursor-default group hover:scale-[1.02] transition-transform"
                        style={{
                          left: `${left}%`,
                          width: `${width}%`,
                          background: it._color,
                          boxShadow: `0 0 10px ${it._color}88`,
                        }}
                      >
                        <span className="truncate">{it.title}</span>
                        <span className="ml-auto text-[11px] opacity-80">{it._dur}h</span>

                        {/* Tooltip */}
                        <div className="absolute left-1/2 -translate-x-1/2 -top-14 bg-slate-900 text-white text-xs p-2 rounded shadow-lg border border-slate-700 opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none">
                          <div className="font-semibold text-sm">{it.title}</div>
                          <div className="text-slate-300">{it.time}</div>
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setEditingItem((prev) => ({ ...editingItem, title: e.target.value }))
}
