...
/* Pre-Flight panel — renders the deterministic constraint reports generated by
   the Python engine. Regenerate with e.g.:
...
const REPORTS = [
  { id: 'gaib',   label: 'Global AI Berlin', file: '/gaib-preflight.json' },
  { id: 'lilium', label: 'Lilium Berlin',    file: '/lilium-preflight.json' },
];
...
export default function PreflightPanel() {
  ...
  useEffect(() => {
    const target = REPORTS.find(r => r.id === selected) || REPORTS[0];
    let cancelled = false;
    setReport(null);
    setError(null);
    fetch(target.file)
    ...
    <div className="flex gap-2">
      {REPORTS.map(r => (
        <button key={r.id} onClick={() => setSelected(r.id)}
          className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${selected === r.id ? 'bg-purple-700 text-white' : 'hover:bg-white/5'}`}
          style={selected === r.id ? {} : { background: 'var(--surface-3)', color: 'var(--text-2)' }}>
          {r.label}
        </button>
      ))}
    </div>
      ...
      <div className="space-y-4">
        ...
        <div className="panel-glass glass-border rounded-md p-8 text-center">
          ...
          <p className="text-xs mt-2" style={{ color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>
            Generate one: python3 agents/event_ops/schemas/&lt;event&gt;.py &gt; public/&lt;event&gt;-preflight.json
          </p>
        </div>
      </div>
    );
  }
  if (!report) return <div className="space-y-4">{selector}<div className="text-sm text-slate-400 p-8">Loading pre-flight report…</div></div>;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Capacity (venues that declare it) */}
      {d.capacity && (
        <div className="panel-glass glass-border rounded-md px-5 py-3 flex items-center justify-between" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          ...
          <span className={`text-sm font-semibold ${d.capacity.over ? 'text-red-300' : d.capacity.expected_attendees >= 0.9 * d.capacity.max_attendees ? 'text-amber-300' : 'text-white'}`}>
            {d.capacity.expected_attendees} expected / {d.capacity.max_attendees} max
          </span>
        </div>
      )}
